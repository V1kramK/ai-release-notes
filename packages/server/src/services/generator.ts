...
import type { EnrichedChange, GeneratedSection, RepoScope } from "@release-notes/shared";
...
export async function generateReleaseNotes(
  ...
  for (const scope of scopes) {
    ...
    try {
      ...
      const jiraResultMap = new Map<string, Awaited<ReturnType<JiraPort["getIssue"]>>>();

      const jiraTasks = uniqueKeys.map((key) => async (): Promise<void> => {
        if (signal.aborted) return;
        try {
        ...
        try {
          ...
          while (!signal.aborted) {
            ...
            if (result.status === "succeeded") {
              const section: GeneratedSection = { category, repo, content: result.text };
          ...
          if (taskId) await summarizer.cancelTask(taskId).catch(() => void 0);
          sendEvent(res, {
            name: "warning",
            data: {
              repo,
...
function buildContext(category: string, repo: string, changes: EnrichedChange[]): string {
  const lines: string[] = [
    `Repository: ${repo}`,
    `Category: ${category}`,
